...
for mod in GE_TOOLS:
    print mod
    exec("import ncTools.tools.graphEditor.tools.{0} as {1}; reload({2})".format(mod, mod, mod))
# -----------------------------------------------------------------------------
# Run
# -----------------------------------------------------------------------------
def run():
...
class ncGraphEditorWidget(QtWidgets.QWidget):
    ...
    def add_graph_editor_toolbar(self):
        ...
        ge_toolbar_layout_qt.layout().addWidget(self.toolbar)

        self.add_sub_toolbars()

    def add_sub_toolbars(self):
        for tool in GE_TOOLS:
            tool_class = "{0}.{1}{2}Toolbar".format(tool, tool[2].upper(), tool[3:])
            print tool_class
            tool_toolbar = eval(tool_class)()
            tool_toolbar_widget = tool_toolbar.create_layout()
            self.toolbar_layout.addWidget(tool_toolbar_widget)
